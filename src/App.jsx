...
// ─── Env config (Vite) ────────────────────────────────────────────────────────
// In artifact/preview mode these fall back to embedded values so the app runs.
const ENV = {
  SB_URL:   import.meta.env?.VITE_SB_URL    ?? "https://qyrqjxbhttaqjgihmzgx.supabase.co",
  SB_KEY:   import.meta.env?.VITE_SB_KEY    ?? "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF5cnFqeGJodHRhcWpnaWhtemd4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE2NTg5MzQsImV4cCI6MjA4NzIzNDkzNH0.cQC2Vo722Z_LY8X5an2QJqJhavjIstmzNbp2Cjo_51I",
  DHAN_TOK: import.meta.env?.VITE_DHAN_ACCESS_TOKEN ?? "",
  DHAN_CID: import.meta.env?.VITE_DHAN_CLIENT_ID   ?? "",
  AO_KEY:   import.meta.env?.VITE_ANGELONE_API_KEY  ?? "",
  AO_CODE:  import.meta.env?.VITE_ANGELONE_CLIENT_CODE ?? "",
  UP_TOK:   import.meta.env?.VITE_UPSTOX_ACCESS_TOKEN ?? "",
};

// ─── Supabase helpers ─────────────────────────────────────────────────────────
const sb = async (path, opts = {}, token = null) => {
  const h = { apikey: ENV.SB_KEY, "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}), ...opts.headers };
  const r = await fetch(`${ENV.SB_URL}${path}`, { ...opts, headers: h });
...
// ─── LTP: Dhan → Angel One → Upstox (broker APIs only, no Yahoo/Claude) ──────

/** Dhan MarketFeed API — batch quote */
async function fetchDhan(symbols, token, clientId) {
  if (!token) return {};
  try {
    // Dhan v2 LTP endpoint (batch)
    ...
    const out = {};
    // Response: { data: { NSE_EQ: { SYMBOL: { last_price, net_change, percent_change } } } }
    ...
    // Fallback: Dhan individual quote endpoint
    const singleOut = {};
    await Promise.allSettled(symbols.slice(0, 15).map(async sym => {
      try {
        const r2 = await fetch(`https://api.dhan.co/v2/marketfeed/quote?NSE_EQ=${sym}`, {
          headers: { "access-token": token, "client-id": clientId || "", Accept: "application/json" },
          signal: AbortSignal.timeout(6000),
        });
        if (!r2.ok) return;
        const d2 = await r2.json();
        const q2 = d2?.data?.NSE_EQ?.[sym] || d2?.data;
        if (q2?.last_price != null) singleOut[sym] = { ltp: q2.last_price, change: q2.net_change??0, pct: q2.percent_change??0 };
      } catch {}
    }));
    return singleOut;
  } catch { return {}; }
...
/** Angel One SmartAPI — batch quote */
async function fetchAngelOne(symbols, apiKey, jwtToken) {
  ...
  try {
    ...
    (data?.data?.fetched || []).forEach(q => {
      if (q.tradingSymbol && q.ltp != null) out[q.tradingSymbol] = { ltp: q.ltp, change: q.netChange??0, pct: q.percentChange??0 };
    });
    return out;
  } catch { return {}; }
}

/** Upstox API v2 — batch quote */
async function fetchUpstox(symbols, accessToken) {
  if (!accessToken) return {};
  try {
    const keys = symbols.map(s => `NSE_EQ|${s}`).join(",");
    const res = await fetch(`https://api.upstox.com/v2/market-quote/ltp?symbol=${encodeURIComponent(keys)}`, {
...
/** Master fetch — Dhan first (env token), then per-user broker keys */
async function fetchAllPrices(symbols, profile) {
  if (!symbols.length) return {};
  const out = {};
  let remaining = [...symbols];

  const merge = data => { Object.assign(out, data); remaining = remaining.filter(s => !out[s]); };

  // 1. Dhan — env token (always available if set)
  if (ENV.DHAN_TOK && remaining.length) merge(await fetchDhan(remaining, ENV.DHAN_TOK, ENV.DHAN_CID));

  // 2. Dhan — user's own token (from settings)
  if (remaining.length && profile?.dhan_access_token) merge(await fetchDhan(remaining, profile.dhan_access_token, profile.dhan_client_id));

  // 3. Angel One — user's API keys
  if (remaining.length && profile?.angelone_api_key && profile?.angelone_client_code)
    merge(await fetchAngelOne(remaining, profile.angelone_api_key, profile.angelone_client_code));

  // 4. Upstox — user's token
  if (remaining.length && profile?.upstox_access_token) merge(await fetchUpstox(remaining, profile.upstox_access_token));

  return out;
}

// ─── Global CSS — hide all scrollbars ────────────────────────────────────────
const globalStyle = `
...
function SettingsPanel({token,onClose,onLogout,isMobile}) {
  ...
  const save=async()=>{setLoading(true);setMsg("");try{await dbUpsert("/rest/v1/user_profiles",{...profile,id:uid,updated_at:new Date().toISOString()},token);setMsg("✓ Saved!");}catch(e){setMsg("Error: "+e.message);}setLoading(false);};
  const changePwd=async()=>{if(!newPwd||newPwd!==newPwd2)return setMsg("Passwords don't match");if(newPwd.length<6)return setMsg("Min 6 characters");setLoading(true);setMsg("");try{await authPwdChange(token,newPwd);setMsg("✓ Password updated!");setNewPwd("");setNewPwd2("");}catch(e){setMsg("Error: "+e.message);}setLoading(false);};
  const enableBio=async()=>{try{const cred=await navigator.credentials.create({publicKey:{challenge:crypto.getRandomValues(new Uint8Array(32)),rp:{name:"KOSH Portfolio",id:window.location.hostname},user:{id:new TextEncoder().encode(uid),name:"kosh-user",displayName:"KOSH User"},pubKeyCredParams:[{type:"public-key",alg:-7}],authenticatorSelection:{authenticatorAttachment:"platform",userVerification:"required"},timeout:60000}});if(cred){const u={...profile,biometric_enabled:true};await dbUpsert("/rest/v1/user_profiles",{...u,id:uid},token);setProfile(u);await stSet("bio_enabled",true);setMsg("✓ Fingerprint registered!");}}catch(e){setMsg("Setup failed: "+e.message);}};
  const disableBio=async()=>{const u={...profile,biometric_enabled:false};await dbUpsert("/rest/v1/user_profiles",{...u,id:uid},token).catch(()=>{});setProfile(u);await stDel("bio_enabled");setMsg("✓ Disabled");};
  ...
  return (<Wrap>
    ...
    {tab==="profile"&&<>
      ...
      <div style={{display:"flex",gap:10,marginTop:4}}>
        ...
        <Btn style={{flex:2}} onClick={save} disabled={loading}>{loading?"Saving…":"Save Profile"}</Btn>
      </div>
    ...
    {tab==="security"&&<>
      ...
      <div style={{background:C.cardL,borderRadius:12,padding:"14px 16px",border:`1px solid ${C.border}`}}>
        ...
        <Row label="New Password"><div style={{position:"relative"}}><Inp type={showPwd?"text":"password"} placeholder="New password" value={newPwd} onChange={e=>setNewPwd(e.target.value)} style={{paddingRight:40}} /><button onClick={()=>setShowPwd(v=>!v)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:14}}>{showPwd?"🙈":"👁"}</button></div></Row>
    ...
    {tab==="api"&&<>
      <div style={{color:C.muted,fontSize:12,marginBottom:14,lineHeight:1.6,background:`${C.navy}22`,border:`1px solid ${C.navy}44`,borderRadius:9,padding:"10px 14px"}}>
        Priority: <span style={{color:C.gold}}>Dhan (env)</span> → <span style={{color:C.dim}}>Dhan (yours)</span> → Angel One → Upstox. Add your keys to improve coverage.
      </div>
      {/* Dhan */}
      <div style={{background:C.cardL,borderRadius:12,padding:"14px 16px",marginBottom:12,border:`1px solid ${C.border}`}}>
        ...
        <Row label="Client ID"><Inp placeholder="Your Dhan Client ID" value={profile.dhan_client_id||""} onChange={e=>setProfile(p=>({...p,dhan_client_id:e.target.value}))} /></Row>
        <div style={{color:C.muted,fontSize:10}}>From dhan.co → My Dhan → API Access</div>
      </div>
      {/* Angel One */}
      <div style={{background:C.cardL,borderRadius:12,padding:"14px 16px",marginBottom:12,border:`1px solid ${C.border}`}}>
        <div style={{color:C.gold,fontWeight:700,fontSize:13,marginBottom:10}}>🅰 Angel One SmartAPI</div>
        <Row label="API Key"><Inp placeholder="Angel One API Key" value={profile.angelone_api_key||""} onChange={e=>setProfile(p=>({...p,angelone_api_key:e.target.value}))} /></Row>
        <Row label="JWT Token"><Inp placeholder="Login JWT token" value={profile.angelone_client_code||""} onChange={e=>setProfile(p=>({...p,angelone_client_code:e.target.value}))} /></Row>
        <div style={{color:C.muted,fontSize:10}}>From marketapi.angelbroking.com → Create App</div>
      </div>
      {/* Upstox */}
      <div style={{background:C.cardL,borderRadius:12,padding:"14px 16px",marginBottom:12,border:`1px solid ${C.border}`}}>
        ...
        <div style={{color:C.muted,fontSize:10}}>From upstox.com/developer → Login API → Access Token</div>
      </div>
      {msg&&<div style={{color:msg.startsWith("✓")?C.profit:C.loss,fontSize:12,marginBottom:8}}>{msg}</div>}
      <Btn onClick={save} disabled={loading} style={{width:"100%"}}>{loading?"Saving…":"Save API Keys"}</Btn>
    </>}
  </Wrap>);
}
...
function AuthScreen({onAuth}) {
  ...
  const [showP,setShowP]=useState(false); const [bioAvail,setBioAvail]=useState(false); const [bioLoading,setBioLoading]=useState(false);

  useEffect(()=>{if(window.PublicKeyCredential?.isUserVerifyingPlatformAuthenticatorAvailable)window.PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable().then(setBioAvail).catch(()=>{});},[]);

  const biometricLogin=async()=>{setBioLoading(true);setErr("");try{const result=await navigator.credentials.get({publicKey:{challenge:crypto.getRandomValues(new Uint8Array(32)),timeout:60000,userVerification:"required",allowCredentials:[]}}).catch(()=>null);if(result){const saved=await stGet("ks");if(saved?.at){onAuth({access_token:saved.at,refresh_token:saved.rt});return;}setErr("No saved session. Sign in first.");}else setErr("Biometric cancelled.");}catch{setErr("Biometric not available.");}setBioLoading(false);};

  const submit=async()=>{if(!email||!pass)return setErr("Fill all fields");setErr("");setLoading(true);try{const d=await(mode==="signup"?authSignup:authLogin)(email,pass);if(mode==="signup"&&!d.access_token){setErr("Confirm your email, then sign in.");setLoading(false);return;}await stSet("ks",{at:d.access_token,rt:d.refresh_token});onAuth(d);}catch(e){setErr(e.message);}setLoading(false);};
    ...
    <div style={{background:C.bg,minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif",padding:16}}>
      ...
      <div style={{width:"100%",maxWidth:360,background:C.card,borderRadius:16,border:`1px solid ${C.border}`,overflow:"hidden",boxShadow:"0 32px 80px rgba(0,0,0,.8)"}}>
        ...
        <div style={{padding:"28px 24px 24px"}}>
          ...
          {bioAvail&&<button onClick={biometricLogin} disabled={bioLoading} style={{width:"100%",background:`${C.navy}55`,border:`1px solid ${C.navy}`,color:C.white,borderRadius:10,padding:"12px",fontWeight:600,fontSize:14,cursor:"pointer",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:"inherit"}}><span style={{fontSize:20}}>🫰</span>{bioLoading?"Verifying…":"Sign in with Fingerprint"}</button>}
          ...
          <button onClick={submit} disabled={loading} style={{width:"100%",background:loading?C.border:C.gold,border:"none",color:C.bg,borderRadius:10,padding:"12px",fontWeight:800,fontSize:14,cursor:loading?"not-allowed":"pointer",fontFamily:"inherit"}}>{loading?"Please wait…":mode==="login"?"Sign In →":"Create Account →"}</button>
...
function StockForm({holding,accounts,token,onSave,onClose}){
  ...
  const submit=async()=>{if(!f.symbol||!f.qty||!f.avg_price||!f.account_id)return setErr("Fill all fields");setLoading(true);setErr("");try{if(holding)await dbPatch(`/rest/v1/holdings?id=eq.${holding.id}`,{account_id:f.account_id,symbol:f.symbol,exchange:f.exchange,qty:+f.qty,avg_price:+f.avg_price},token);else await dbPost("/rest/v1/holdings",{user_id:uid,account_id:f.account_id,symbol:f.symbol,exchange:f.exchange,qty:+f.qty,avg_price:+f.avg_price,asset_type:"stock"},token);onSave();}catch(e){setErr(e.message);}setLoading(false);};
  return(<><Row label="Account"><Sel value={f.account_id} onChange={e=>set("account_id",e.target.value)}>{accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</Sel></Row><Row label="Exchange"><Sel value={f.exchange} onChange={e=>set("exchange",e.target.value)}><option>NSE</option><option>BSE</option></Sel></Row><Row label="Symbol / Company"><SearchInput placeholder="RELIANCE, Infosys, HAL…" value={text} onChange={v=>{setText(v.toUpperCase());set("symbol",v.toUpperCase());search(v);}} results={res} onSelect={s=>{set("symbol",s.symbol);setText(s.symbol);setRes([]);}} renderResult={s=><div><span style={{color:C.gold,fontWeight:700}}>{s.symbol}</span><span style={{color:C.dim,fontSize:11,marginLeft:6}}>{s.company_name}</span><span style={{float:"right",color:C.muted,fontSize:9,background:C.cardL,borderRadius:4,padding:"1px 5px"}}>{s.market_cap_category}</span></div>} /></Row><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Row label="Qty"><Inp type="number" placeholder="100" value={f.qty} onChange={e=>set("qty",e.target.value)} /></Row><Row label="Avg Price (₹)"><Inp type="number" placeholder="250.50" value={f.avg_price} onChange={e=>set("avg_price",e.target.value)} /></Row></div>{f.qty&&f.avg_price&&<div style={{background:C.bg,borderRadius:8,padding:"10px 14px",border:`1px solid ${C.gold}33`,display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.muted,fontSize:12}}>Invested</span><span style={{color:C.gold,fontWeight:700}}>₹{fmt(+f.qty*+f.avg_price)}</span></div>}{err&&<div style={{color:C.loss,fontSize:12,marginTop:8,padding:"6px 10px",background:"#EF444411",borderRadius:6}}>{err}</div>}<div style={{display:"flex",gap:10,marginTop:18}}><Btn variant="ghost" style={{flex:1}} onClick={onClose}>Cancel</Btn><Btn style={{flex:2}} onClick={submit} disabled={loading}>{loading?"Saving…":holding?"Update":"Add Stock"}</Btn></div></>);
}
...
function MFForm({holding,accounts,token,onSave,onClose}){
  ...
  const submit=async()=>{if(!f.scheme_name||!f.units||!f.avg_nav||!f.account_id)return setErr("Fill all fields");setLoading(true);setErr("");try{if(holding)await dbPatch(`/rest/v1/mf_holdings?id=eq.${holding.id}`,{account_id:f.account_id,scheme_name:f.scheme_name,isin:f.isin,fund_house:f.fund_house,fund_type:f.fund_type,units:+f.units,avg_nav:+f.avg_nav},token);else await dbPost("/rest/v1/mf_holdings",{user_id:uid,account_id:f.account_id,scheme_name:f.scheme_name,isin:f.isin,fund_house:f.fund_house,fund_type:f.fund_type,units:+f.units,avg_nav:+f.avg_nav},token);onSave();}catch(e){setErr(e.message);}setLoading(false);};
  return(<><Row label="Account"><Sel value={f.account_id} onChange={e=>set("account_id",e.target.value)}>{accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</Sel></Row><Row label="Fund Type"><Sel value={f.fund_type} onChange={e=>set("fund_type",e.target.value)}>{MF_TYPES.map(t=><option key={t}>{t}</option>)}</Sel></Row><Row label="Scheme Name"><SearchInput placeholder="HDFC Flexi Cap, SBI Small Cap…" value={text} onChange={v=>{setText(v);set("scheme_name",v);search(v);}} results={res} onSelect={s=>{set("scheme_name",s.scheme_name);set("isin",s.isin);set("fund_house",s.fund_house);set("fund_type",s.fund_type);setText(s.scheme_name);setRes([]);}} renderResult={s=><div><div style={{color:C.white,fontWeight:600,fontSize:12}}>{s.scheme_name}</div><div style={{color:C.muted,fontSize:10,marginTop:2}}>{s.fund_house} · {s.fund_type}</div></div>} /></Row><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Row label="Units"><Inp type="number" step="0.001" placeholder="50.234" value={f.units} onChange={e=>set("units",e.target.value)} /></Row><Row label="Avg NAV (₹)"><Inp type="number" step="0.01" placeholder="450.00" value={f.avg_nav} onChange={e=>set("avg_nav",e.target.value)} /></Row></div>{f.units&&f.avg_nav&&<div style={{background:C.bg,borderRadius:8,padding:"10px 14px",border:`1px solid ${C.gold}33`,display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.muted,fontSize:12}}>Invested</span><span style={{color:C.gold,fontWeight:700}}>₹{fmt(+f.units*+f.avg_nav)}</span></div>}{err&&<div style={{color:C.loss,fontSize:12,marginTop:8}}>{err}</div>}<div style={{display:"flex",gap:10,marginTop:18}}><Btn variant="ghost" style={{flex:1}} onClick={onClose}>Cancel</Btn><Btn style={{flex:2}} onClick={submit} disabled={loading}>{loading?"Saving…":holding?"Update":"Add Fund"}</Btn></div></>);
}
...
function ETFForm({holding,accounts,token,onSave,onClose}){
  ...
  const submit=async()=>{if(!f.symbol||!f.units||!f.avg_price||!f.account_id)return setErr("Fill all fields");setLoading(true);setErr("");try{if(holding)await dbPatch(`/rest/v1/etf_holdings?id=eq.${holding.id}`,{account_id:f.account_id,symbol:f.symbol,etf_name:f.etf_name,etf_type:f.etf_type,units:+f.units,avg_price:+f.avg_price},token);else await dbPost("/rest/v1/etf_holdings",{user_id:uid,account_id:f.account_id,symbol:f.symbol,etf_name:f.etf_name,etf_type:f.etf_type,exchange:"NSE",units:+f.units,avg_price:+f.avg_price},token);onSave();}catch(e){setErr(e.message);}setLoading(false);};
  return(<><Row label="Account"><Sel value={f.account_id} onChange={e=>set("account_id",e.target.value)}>{accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</Sel></Row><Row label="ETF Type"><Sel value={f.etf_type} onChange={e=>set("etf_type",e.target.value)}>{ETF_TYPES.map(t=><option key={t}>{t}</option>)}</Sel></Row><Row label="ETF Symbol"><SearchInput placeholder="NIFTYBEES, GOLDBEES…" value={text} onChange={v=>{setText(v.toUpperCase());set("symbol",v.toUpperCase());search(v);}} results={res} onSelect={s=>{set("symbol",s.symbol);set("etf_name",s.etf_name);set("etf_type",s.etf_type);setText(s.symbol);setRes([]);}} renderResult={s=><div><span style={{color:C.gold,fontWeight:700}}>{s.symbol}</span><span style={{color:C.dim,fontSize:11,marginLeft:6}}>{s.etf_name}</span></div>} /></Row><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Row label="Units"><Inp type="number" step="0.001" placeholder="10" value={f.units} onChange={e=>set("units",e.target.value)} /></Row><Row label="Avg Price (₹)"><Inp type="number" step="0.01" placeholder="250.00" value={f.avg_price} onChange={e=>set("avg_price",e.target.value)} /></Row></div>{f.units&&f.avg_price&&<div style={{background:C.bg,borderRadius:8,padding:"10px 14px",border:`1px solid ${C.gold}33`,display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.muted,fontSize:12}}>Invested</span><span style={{color:C.gold,fontWeight:700}}>₹{fmt(+f.units*+f.avg_price)}</span></div>}{err&&<div style={{color:C.loss,fontSize:12,marginTop:8}}>{err}</div>}<div style={{display:"flex",gap:10,marginTop:18}}><Btn variant="ghost" style={{flex:1}} onClick={onClose}>Cancel</Btn><Btn style={{flex:2}} onClick={submit} disabled={loading}>{loading?"Saving…":holding?"Update":"Add ETF"}</Btn></div></>);
}
...
function AccForm({account,token,onSave,onClose}){
  ...
  return(<><Row label="Account Name"><Inp placeholder="e.g. Zerodha Primary" value={name} onChange={e=>setName(e.target.value)} /></Row><Row label="Broker"><Sel value={broker} onChange={e=>setBroker(e.target.value)}>{BROKERS.map(b=><option key={b}>{b}</option>)}</Sel></Row><Row label="Color Tag"><div style={{display:"flex",flexWrap:"wrap",gap:10,marginTop:4}}>{ACC_COLORS.map(c=><div key={c} onClick={()=>setColor(c)} style={{width:28,height:28,borderRadius:"50%",background:c,cursor:"pointer",border:`3px solid ${color===c?C.white:"transparent"}`,boxSizing:"border-box"}} />)}</div></Row>{err&&<div style={{color:C.loss,fontSize:12}}>{err}</div>}<div style={{display:"flex",gap:10,marginTop:20}}><Btn variant="ghost" style={{flex:1}} onClick={onClose}>Cancel</Btn><Btn style={{flex:2}} onClick={submit} disabled={loading}>{loading?"Saving…":account?"Update":"Add Account"}</Btn></div></>);
}
...
export default function App() {
  ...
  useEffect(()=>{(async()=>{const s=await stGet("ks");if(s?.at){if(jwtExp(s.at)>Date.now()+60000){setSession(s);setChecking(false);return;}if(s.rt){try{const r=await authRefresh(s.rt);const fresh={at:r.access_token,rt:r.refresh_token};await stSet("ks",fresh);setSession(fresh);setChecking(false);return;}catch{}}}setChecking(false);})();});
  if(checking)return<div style={{background:C.bg,height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui",color:C.gold,letterSpacing:4,fontSize:14}}><style>{globalStyle}</style>KOSH…</div>;
...
function Main({session,onLogout}){
  ...
  const liveRef=useRef(null); const marketOpen=isMarketOpen();
  ...
  // ── Compact account card for sidebar ─────────────────────────────────────
  const AccCard=({acc})=>{
    const isA=activeAcc===acc.id;
    return(<div onMouseEnter={()=>setHovRow(`a${acc.id}`)} onMouseLeave={()=>setHovRow(null)} onClick={()=>setActiveAcc(acc.id)} style={{padding:"7px 10px",cursor:"pointer",borderLeft:`3px solid ${isA?acc.color:"transparent"}`,background:isA?`${acc.color}18`:"transparent",borderBottom:`1px solid ${C.border}`,transition:"all .1s"}}>
      ...
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          ...
          <tbody>{curRows.map((h,i)=>{const acc=accounts.find(a=>a.id===h.account_id);const isH=hovRow===h.id;return(<tr key={h.id} onMouseEnter={()=>setHovRow(h.id)} onMouseLeave={()=>setHovRow(null)} style={{borderBottom:`1px solid ${C.border}`,background:isH?`${C.navy}22`:i%2?`${C.bg}66`:"transparent"}}>
            ...
            <td style={{...TD,color:C.dim}}>{ph(fmtC(h.inv),privacy)}</td><td style={{...TD,color:C.white,fontWeight:600}}>{ph(h.cur!=null?fmtC(h.cur):"—",privacy)}</td><td style={TD}><PnlBadge pnl={h.pnl} privacy={privacy} /></td><td style={{...TD,color:privacy?C.muted:clr(h.pct),fontWeight:600}}>{privacy?HIDDEN:h.pct!=null?`${h.pct>=0?"+":""}${fmt(h.pct)}%`:"—"}</td>
  ...
  return(
    <div style={{background:C.bg,height:"100vh",fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <header style={{background:C.card,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",padding:"0 16px",minHeight:56,flexShrink:0,gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:9,paddingRight:12,borderRight:`1px solid ${C.border}`}}>
          <KoshLogo size={30} />
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <aside style={{width:162,background:C.card,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0}}>
          ...
          {/* Accounts list — overflow hidden, no scrollbar */}
          <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
            {accSummary.map(acc=><AccCard key={acc.id} acc={acc} />)}
          </div>
          <div style={{padding:7,borderTop:`1px solid ${C.border}`,flexShrink:0}}>
        ...
        <main style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{padding:"5px 16px",borderTop:`1px solid ${C.border}`,background:C.card,display:"flex",justifyContent:"center",alignItems:"center",gap:6,flexShrink:0}}>
            <KoshLogo size={16} /><span style={{color:C.muted,fontSize:9,letterSpacing:2}}>EXCLUSIVELY POWERED BY KOSH</span>
          </div>
        </main>
      </div>
